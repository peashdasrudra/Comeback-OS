const Body = (props) => {
  const { workoutDay, setWorkoutDay, dayPct, exPct, isSetDone, setTasksDone, setShowExForm, showExForm, warmupDone, setWarmupDone, streak, timer, setTimer, gainXP, T, orb, mono, raj, C, DAYS, Ring, RestTimer } = props;
        ...
        <div style={{ ...C({ padding: "16px", marginBottom: 10, border: `1px solid ${d.color}44` }) }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ ...orb, fontSize: 17, fontWeight: 900, ...{ fontFamily: "'Orbitron',monospace", color: d.color } }}>{d.icon} {d.name}</div>
            ...
            <div key={ex.id} style={{ ...C({ padding: "14px", marginBottom: 10, border: `1px solid ${exDone ? d.color + "55" : T.border}`, background: exDone ? d.color + "06" : T.bg1, transition: "all .3s" }) }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {Array.from({ length: ex.sets }, (_, si) => {
                  ...
                  return (
                    <button key={si} className="btn-tap" onClick={() => {
                      setTasksDone(p => ({ ...p, [`set-${workoutDay}-${ex.id}-${si}`]: !sd }));
                      if (!sd && ex.rest > 0 && si < ex.sets - 1) setTimer({ seconds: ex.rest });
                      if (!sd) gainXP(2, "Set completed 💪");
                    }} style={{ padding: "8px 16px", borderRadius: 7, cursor: "pointer", ...orb, fontSize: 10, fontWeight: 700, transition: "all .2s", border: `1.5px solid ${sd ? d.color : T.border}`, background: sd ? d.color + "33" : T.bg2, color: sd ? d.color : T.muted, boxShadow: sd ? `0 0 8px ${d.color}44` : "none" }}>
                      {sd ? "✓ " : ""}SET {si + 1}
                    </button>
                  );
                })}
