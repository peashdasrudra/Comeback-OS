...
const Plan = (props) => {
  ...
  const Sect = ({ title, emoji, color: sc, items, cat }) => (
    <div style={{ ...C({ padding: "14px", marginBottom: 10, border: `1px solid ${sc}22` }) }}>
      ...
      {items.map((task, i) => {
        const done = tasksDone[`${w.week}-${cat}-${i}`];
        const nk = `${w.week}-${cat}-${i}`;
        return (
          <div key={i} style={{ marginBottom: i < items.length - 1 ? 10 : 0 }}>
            <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
              <div onClick={() => {
                setTasksDone(p => ({ ...p, [nk]: !p[nk] }));
                if (!done) {
                  const xpMap = { thesis: 10, csprep: 8, ielts: 5, admission: 12 };
                  const xpAmt = xpMap[cat] || 5;
                  // Note: gainXP needs to be passed as prop if we want XP
                }
              }} style={{ width: 20, height: 20, border: `1.5px solid ${done ? sc : sc + "44"}`, borderRadius: 4, flexShrink: 0, marginTop: 2, cursor: "pointer", background: done ? sc + "33" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: done ? `0 0 8px ${sc}44` : "none", transition: "all .2s" }}>
